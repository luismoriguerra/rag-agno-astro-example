...
- Q: Can users delete research sessions in v1? → A: No. Delete is deferred; sessions persist for the owner.
...
- **FR-022**: The research agent MUST perform multiple web searches per article (at least one search per planned section/theme) using DuckDuckGoTools. Each article MUST cite a minimum of 3 source URLs in the source list. When search snippets are insufficient, the agent MUST flag the gap in chat (FR-015) rather than fabricating content. No paid search API or second provider is required for v1.
...
- **Deployment and Observability**: The feature requires new API routes and a dedicated research agent configuration. Environment variables must include OpenRouter API access and `research_agent_model=deepseek/deepseek-v4-pro:nitro` (or equivalent), plus any external search API keys. Health checks should cover the new endpoints. Agent traces should be captured via existing LangWatch integration with model id recorded per run.
...
- v1 uses Agno DuckDuckGoTools only for web research (multi-search per article, cited URLs, gap flagging). Full-page fetch, paywalled sources, and paid APIs (Tavily, Exa, etc.) are out of scope for v1 and may be evaluated if evidence quality proves insufficient.
- The research agent uses OpenRouter `deepseek/deepseek-v4-pro:nitro` for long-context article generation; the general chat agent continues using the existing configured model.
...
- Session delete/archive is out of scope for v1.
